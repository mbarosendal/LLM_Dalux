...
def register_dalux_tools_tasks(mcp: FastMCP, adapter: DaluxAdapter) -> None:
    ...
    # Endpoint for a single task does not actually provide much more value than the collection endpoint with filters, and it adds
    # complexity and risk around taskId usage, so we will skip it for now and add if needed based on user feedback.
    # @mcp.tool()
